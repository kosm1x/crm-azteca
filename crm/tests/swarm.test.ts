...
const { getRecipe, getRecipesForRole, RECIPES, truncateResult } = await import('../src/tools/swarm-recipes.js');
...
function seedProposals() {
  ...
  // Active proposals
  testDb.prepare(`INSERT INTO propuesta (id, cuenta_id, ae_id, titulo, valor_estimado, medios, tipo_oportunidad, etapa, fecha_creacion, fecha_ultima_actividad, dias_sin_actividad)
    VALUES ('p1', 'c1', 'ae-001', 'TV Navidad', 10000000, '{"tv_abierta":7000000,"ctv":3000000}', 'estacional', 'en_negociacion', ?, ?, 3)`).run(recent, now);
  testDb.prepare(`INSERT INTO propuesta (id, cuenta_id, ae_id, titulo, valor_estimado, medios, tipo_oportunidad, etapa, fecha_creacion, fecha_ultima_actividad, dias_sin_actividad)
    VALUES ('p2', 'c2', 'ae-002', 'Digital Bimbo', 5000000, '{"digital":5000000}', 'lanzamiento', 'enviada', ?, ?, 10)`).run(recent, now);
  ...
  testDb.prepare(`INSERT INTO propuesta (id, cuenta_id, ae_id, titulo, valor_estimado, tipo_oportunidad, etapa, fecha_creacion, fecha_ultima_actividad)
    VALUES ('p3', 'c1', 'ae-001', 'Radio Verano', 3000000, 'estacional', 'completada', ?, ?)`).run(recent, now);
  testDb.prepare(`INSERT INTO propuesta (id, cuenta_id, ae_id, titulo, valor_estimado, tipo_oportunidad, etapa, razon_perdida, fecha_creacion, fecha_ultima_actividad)
    VALUES ('p4', 'c2', 'ae-002', 'CTV Bimbo', 2000000, 'prospeccion', 'perdida', 'precio', ?, ?)`).run(recent, now);
}
...
function seedCuotas() {
  ...
  for (let w = Math.max(1, currentWeek - 3); w <= currentWeek; w++) {
    testDb.prepare(`INSERT INTO cuota (id, persona_id, rol, año, semana, meta_total, logro)
      VALUES (?, 'ae-001', 'ae', ?, ?, 5000000, ?)`).run(`q1-${w}`, year, w, 3500000 + w * 50000);
    testDb.prepare(`INSERT INTO cuota (id, persona_id, rol, año, semana, meta_total, logro)
      VALUES (?, 'ae-002', 'ae', ?, ?, 5000000, ?)`).run(`q2-${w}`, year, w, 4000000 - w * 50000);
  }
}
...
function seedActivities() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    testDb.prepare(`INSERT INTO actividad (id, ae_id, cuenta_id, tipo, resumen, sentimiento, fecha)
      VALUES (?, ?, 'c1', ?, ?, ?, ?)`).run(`act-${i}`, aeId, tipo, `Actividad ${i}`, sentimiento, date);
  }
}

function seedInventario() {
  testDb.prepare(`INSERT INTO inventario (id, medio, propiedad, formato, precio_referencia) VALUES ('inv1', 'tv_abierta', 'Canal Uno', 'Spot 20s', 50000)`).run();
  testDb.prepare(`INSERT INTO inventario (id, medio, propiedad, formato, precio_referencia) VALUES ('inv2', 'ctv', 'Azteca Play', 'Pre-roll', 30000)`).run();
  testDb.prepare(`INSERT INTO inventario (id, medio, propiedad, formato, precio_referencia) VALUES ('inv3', 'radio', 'Radio Fórmula', 'Spot 30s', 15000)`).run();
  testDb.prepare(`INSERT INTO inventario (id, medio, propiedad, formato, precio_referencia) VALUES ('inv4', 'digital', 'Web', 'Banner', 10000)`).run();
}
...
describe('ejecutar_swarm — error handling', () => {
  ...
  it('returns error for unknown recipe', async () => {
    const ctx = makeCtx('gerente', 'mgr-001');
    const result = JSON.parse(await ejecutar_swarm({ receta: 'nonexistent' }, ctx));
  ...
  it('returns error when role not authorized for recipe', async () => {
    const ctx = makeCtx('gerente', 'mgr-001');
    const result = JSON.parse(await ejecutar_swarm({ receta: 'resumen_ejecutivo' }, ctx));
  ...
  it('diagnostico_persona requires persona_nombre', async () => {
    seedAll();
    const ctx = makeCtx('gerente', 'mgr-001');
    const result = JSON.parse(await ejecutar_swarm({ receta: 'diagnostico_persona' }, ctx));
...
describe('resumen_semanal_equipo', () => {
  it('returns valid structure with data', async () => {
    seedAll();
    const ctx = makeCtx('gerente', 'mgr-001');
    const result = JSON.parse(await ejecutar_swarm({ receta: 'resumen_semanal_equipo' }, ctx));
...
describe('diagnostico_persona', () => {
  it('returns analysis for a specific person', async () => {
    seedAll();
    const ctx = makeCtx('gerente', 'mgr-001');
    const result = JSON.parse(await ejecutar_swarm({ receta: 'diagnostico_persona', persona_nombre: 'Carlos Lopez' }, ctx));
...
describe('comparar_equipo', () => {
  it('returns team comparison data', async () => {
    seedAll();
    const ctx = makeCtx('gerente', 'mgr-001');
    const result = JSON.parse(await ejecutar_swarm({ receta: 'comparar_equipo' }, ctx));
...
describe('diagnostico_medio', () => {
  it('returns per-medio breakdown', async () => {
    seedAll();
    const ctx = makeCtx('vp', 'vp-001');
    const result = JSON.parse(await ejecutar_swarm({ receta: 'diagnostico_medio' }, ctx));
...
describe('truncateResult', () => {
  ...
  it('truncates large datos entries', () => {
    ...
    const result = { receta: 'test', resumen: 'ok', datos: { big1: bigData, big2: bigData, small: 'hello' } };
